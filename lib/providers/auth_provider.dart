...
import 'package:tasq/ui/widgets/shared/single_field_edit_dialog.dart';
...
// google sign-in
  ...
  void searchOrganization() async {
    startLoader();
    allOrganizations = await fs.getAllOrganizations();
    stopLoader();
    AppRoutes.push(Get.context, SearchOrganization());
  }

  // edit name on sign up
  void editName() async {
    String newName = await Get.dialog(SingleFieldEditDialog(
      title: "edit name",
    ));
    if (newName != null) {
      userDataToSet.name = newName;
      stopLoader();
    }
  }

  void sendOrgJoinRequest({OrgMember member}) async {
    startLoader();
    await fs.sendOrgJoinRequest(member: member);
    stopLoader();
  }
...
// and navigate according to the role.
    ...
    if (userData == null) {
      ...
      AppRoutes.makeFirst(Get.context, RoleSelection());
    } else {
  ...
  void registerOrg(String orgName, File cropped) async {
    ...
    Organization myOrg = await fs.getMyOrg();
    if (myOrg != null) {
      AppUser.organization = myOrg;
    }
    croppedFile = null;
