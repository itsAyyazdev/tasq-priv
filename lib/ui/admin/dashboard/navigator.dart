...
class _OrgDashboardState extends State<OrgDashboard> {
  ...
  Widget build(BuildContext context) {
    return Consumer<AppProvider>(
        builder: (context, p, _) => Scaffold(
              ...
              body: PageView(
                controller: AppUtils.adminPageController,
                children: [
                  AdminHome(),
                  AdminRewards(),
                  AdminPreferences(),
  ...
  void onTap(int i, AppProvider p) {
    ...
    print(i);
    AppUtils.adminPageController
        .animateToPage(i, duration: duration, curve: Curves.decelerate);
    p.setPageTitle(
        i == 0 ? AppUser.organization?.name ?? "home" : bottomNames[i],
        duration);
  }
}
